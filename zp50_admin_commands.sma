...
new g_MaxPlayers
new g_GameModeInfectionID, g_GameModeNemesisID, g_GameModeDepredadorID, g_GameModeSurvivorID
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library))
		register_concmd("zp_depredador", "cmd_depredador", _, "<target> - Turn someone into a Depredador", 0)
...
public plugin_natives()
{
	...
	register_native("zp_admin_commands_depredador", "native_admin_commands_depredador")
...
public plugin_cfg()
{
	...
	g_GameModeDepredadorID = zp_gamemodes_get_id("Depredador Mode")
...
public native_admin_commands_depredador(plugin_id, num_params)
{
	new id_admin = get_param(1)
	...
	command_depredador(id_admin, player)
	return true;
}
...
{
	// Check for access flag - Make Nemesis
	if (!cmd_access(id, read_flags(g_access_make_depredador), cid, 2))
		return PLUGIN_HANDLED;
	...
	// Target not allowed to be nemesis
	if (zp_class_depredador_get(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP] %L (%s).", id, "ALREADY_DEPREDADOR", player_name)
		return PLUGIN_HANDLED;
	}
...
command_depredador(id, player)
{
	...
	{
		// Nemesis mode disabled
		if (g_GameModeDepredadorID == ZP_INVALID_GAME_MODE)
		{
			zp_colored_print(id, "%L", id, "CMD_ONLY_AFTER_GAME_MODE")
			return;
		}
		...
		// Start nemesis game mode with this target player
		if (!zp_gamemodes_start(g_GameModeDepredadorID, player))
		{
			zp_colored_print(id, "%L", id, "GAME_MODE_CANT_START")
			return;
		}
	...
	{
		// Make player nemesis
		zp_class_depredador_set(player)
	}
	...
	if (cvar_amx_show_activity)
	{
		switch (get_pcvar_num(cvar_amx_show_activity))
		{
			case 1: zp_colored_print(0, "ADMIN - %s %L", player_name, LANG_PLAYER, "CMD_DEPREDADOR")
			case 2: zp_colored_print(0, "ADMIN %s - %s %L", admin_name, player_name, LANG_PLAYER, "CMD_DEPREDADOR")
		}
	}
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		get_user_authid(id, authid, charsmax(authid))
		get_user_ip(id, ip, charsmax(ip), 1)
		zp_log("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_DEPREDADOR", GetPlayingCount())
	}
}
