...
new g_access_make_depredador[ACCESSFLAG_MAX_LENGTH] = "d"
...
enum
{
	...
	ACTION_MAKE_SUPERCLASS,
	ACTION_RESPAWN_PLAYER,
	ACTION_START_GAME_MODE,

	ACTION_MAKE_NEMESIS,
	ACTION_MAKE_DEPREDADOR,
...
public plugin_init()
{
	...
	register_menu("Super Menu", KEYSMENU, "menu_super")
...
public plugin_precache()
{
	...
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Access Flags", "MAKE DEPREDADOR", g_access_make_depredador, charsmax(g_access_make_depredador)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Access Flags", "MAKE DEPREDADOR", g_access_make_depredador)
...
show_menu_admin(id)
{
	...
	// 2. Menu unificado
	len += formatex(menu[len], charsmax(menu) - len, "\r2.\w Convertir en SuperClase^n", id)
	...
	if (userflags & read_flags(g_access_respawn_players))
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w %L^n", id, "MENU_ADMIN4")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. %L^n", id, "MENU_ADMIN4")
	
	// 5. Start Game Mode command
	if (userflags & read_flags(g_access_start_game_mode))
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w %L^n", id, "MENU_ADMIN_START_GAME_MODE")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. %L^n", id, "MENU_ADMIN_START_GAME_MODE")
...
show_menu_player_list(id)
{
	...
	switch (PL_ACTION)
	{
		...
		case ACTION_MAKE_DEPREDADOR: formatex(menu, charsmax(menu), "%L\r", id, "MENU_DEPREDADOR")
	...
	for (player = 0; player <= g_MaxPlayers; player++)
	{
		...
		new szClass[32], szColor[3]
		new bool:is_zombie = zp_core_is_zombie(player)
		...
		switch (PL_ACTION)
		{
			...
			{
				get_special_class_name(player, is_zombie, szClass, charsmax(szClass))

				if (is_zombie)
					copy(szColor, charsmax(szColor), ((userflags & read_flags(g_access_make_human)) && is_user_alive(player)) ? "\r" : "\d")
				else
					copy(szColor, charsmax(szColor), ((userflags & read_flags(g_access_make_zombie)) && is_user_alive(player)) ? "\y" : "\d")

				formatex(menu, charsmax(menu), "%s %s[%L]", player_name, szColor, id, szClass)
			}
			case ACTION_MAKE_NEMESIS: // Nemesis command
			{
				...
				if (LibraryExists(LIBRARY_NEMESIS, LibType_Library)
					&& (userflags & read_flags(g_access_make_nemesis))
					&& is_user_alive(player)
					&& !zp_class_nemesis_get(player))
				{
					copy(szColor, charsmax(szColor), is_zombie ? "\r" : "\y")
				}
				else
				{
					copy(szColor, charsmax(szColor), "\d")
				}

				formatex(menu, charsmax(menu), "%s %s[%L]", player_name, szColor, id, szClass)
			}
			case ACTION_MAKE_DEPREDADOR: // Depredador command
			{
				get_special_class_name(player, is_zombie, szClass, charsmax(szClass))

				if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library)
					&& (userflags & read_flags(g_access_make_depredador))
					&& is_user_alive(player)
					&& !zp_class_depredador_get(player))
				{
					copy(szColor, charsmax(szColor), is_zombie ? "\r" : "\y")
				}
				else
			...
			case ACTION_MAKE_SURVIVOR: // Survivor command
			{
				...
				if (LibraryExists(LIBRARY_SURVIVOR, LibType_Library)
					&& (userflags & read_flags(g_access_make_survivor))
					&& is_user_alive(player)
					&& !zp_class_survivor_get(player))
				{
					copy(szColor, charsmax(szColor), is_zombie ? "\r" : "\y")
				}
				else
...
public menu_admin(id, key)
{
	...
	switch (key)
	{
		...
		case ACTION_MAKE_SUPERCLASS:
		{
			if ((userflags & (read_flags(g_access_make_nemesis) && read_flags(g_access_make_depredador) && read_flags(g_access_make_survivor))))
			{
				// Show player list for admin to pick a target
				PL_ACTION = ACTION_MAKE_SUPERCLASS
				menu_super(id)
			}
			else
			{
...
public menu_super_handler(id, menu, item)
{
    if (item == MENU_EXIT)
    {
        menu_destroy(menu)
        show_menu_admin(id) // Volver al menú principal
        return PLUGIN_HANDLED
    }
    ...
    switch (key)
    {
		...
		case 1:
		{
            PL_ACTION = ACTION_MAKE_DEPREDADOR
            show_menu_player_list(id)
		}
        case 2: // Survivor
        {
...
public menu_super(id)
{
    new menu = menu_create("Selecciona la SuperClase:", "menu_super_handler")

    menu_additem(menu, "Converir Nemesis", "0")
	menu_additem(menu, "Converir Depredador", "1")
    menu_additem(menu, "Converir Survivor", "2")
...
public menu_player_list(id, menuid, item)
{
    ...
    if (item == MENU_EXIT)
    {
        ...
        // Volver al menú adecuado según la acción actual
        switch (PL_ACTION)
        {
            case ACTION_MAKE_NEMESIS, ACTION_MAKE_SURVIVOR, ACTION_MAKE_DEPREDADOR:
            {
                menu_super(id) // Volver al menú de superclases
            }
            default:
            {
                show_menu_admin(id) // Volver al menú de administración normal
            }
        }
	...
	if (is_user_connected(player))
	{
		...
		switch (PL_ACTION)
		{
			...
			case ACTION_MAKE_DEPREDADOR: // Nemesis command
			{
				if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library) && (userflags & read_flags(g_access_make_depredador)) && is_user_alive(player) && !zp_class_depredador_get(player))
					zp_admin_commands_depredador(id, player)
				else
					zp_colored_print(id, "%L", id, "CMD_NOT")
			}
			case ACTION_MAKE_SURVIVOR: // Survivor command
...
get_special_class_name(id, bool:is_zombie, szClass[], len)
{
	if (is_zombie)
	{
		if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library) && zp_class_depredador_get(id))
		{
			copy(szClass, len, "CLASS_DEPREDADOR")
			return
		}
		if (LibraryExists(LIBRARY_NEMESIS, LibType_Library) && zp_class_nemesis_get(id))
		{
			copy(szClass, len, "CLASS_NEMESIS")
			return
		}
		copy(szClass, len, "CLASS_ZOMBIE")
	}
	else
	{
		if (LibraryExists(LIBRARY_SURVIVOR, LibType_Library) && zp_class_survivor_get(id))
		{
			copy(szClass, len, "CLASS_SURVIVOR")
			return
		}
		copy(szClass, len, "CLASS_HUMAN")
	}
}