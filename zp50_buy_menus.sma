...
bool:is_primary_weapon(weaponid)
{
	return ((1 << weaponid) & PRIMARY_WEAPONS_BIT_SUM) != 0;
}

bool:is_secondary_weapon(weaponid)
{
	return ((1 << weaponid) & SECONDARY_WEAPONS_BIT_SUM) != 0;
}

bool:player_has_weapon_of_type(id, weaponid, filter_type)
{
	new weapons[32], num_weapons
	...
	for (new i = 0; i < num_weapons; i++)
	{
		...
		{
			if (filter_type == PRIMARY_ONLY && is_primary_weapon(weaponid))
				return true;
			else if (filter_type == SECONDARY_ONLY && is_secondary_weapon(weaponid))
				return true;
		}
	}
...
buy_primary_weapon(id, selection)
{
	...
	new bool:has_weapon = player_has_weapon_of_type(id, weaponid, PRIMARY_ONLY)
	new bool:has_super_weapon = zp_super_weapon_prim(id)

	if (is_user_bot(id))
		// Bots nunca dropean armas, solo eliminan
		strip_weapons(id, PRIMARY_ONLY);
	else
	{
		if (has_super_weapon)
		{
			//server_print("[DEBUG] Tenías arma especial. Dropeando.")
		...
		else
		{
			if (has_weapon)
			{
				//server_print("[DEBUG] Tenías primaria común. Strip.")
				strip_weapons(id, PRIMARY_ONLY)
			}
			else
			{
				//server_print("[DEBUG] Tenías otra primaria. Dropeando.")
				drop_weapons(id, PRIMARY_ONLY)
			}
		}
	}
...
buy_secondary_weapon(id, selection)
{
	...
	new weaponid = get_weaponid(weapon_name)
	new bool:has_weapon = player_has_weapon_of_type(id, weaponid, SECONDARY_ONLY)
	new bool:has_super_weapon = zp_super_weapon_sec(id)
...
public zp_super_weapon_prim(id)
{
    new bool:has_gmp5 = zp_mp5_user_get(id);
    //server_print("[DEBUG] zp_super_weapon_prim: has_gmp5 = %d", has_gmp5);
    return has_gmp5;
}

public zp_super_weapon_sec(id)
{
    new bool:has_gmp5 = false;
    //server_print("[DEBUG] zp_super_weapon_prim: has_gmp5 = %d", has_gmp5);
    return has_gmp5;
}
