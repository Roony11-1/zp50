...
#define HUMANS_DEFAULT_SHORT_DESCRIPTION "Default"
#define HUMANS_DEFAULT_HEALTH 100
#define HUMANS_DEFAULT_MAX_ARMOR 100
...
new Array:g_HumanClassShortDesc
new Array:g_HumanClassHealth
new Array:g_HumanClassMaxArmor
...
new g_HumanClassViewing[MAX_PLAYERS + 1]
...
public plugin_cfg()
{
	...
	if (g_HumanClassCount < 1)
	{
		...
		ArrayPushString(g_HumanClassShortDesc, HUMANS_DEFAULT_SHORT_DESCRIPTION)
		ArrayPushCell(g_HumanClassHealth, HUMANS_DEFAULT_HEALTH)
		ArrayPushCell(g_HumanClassMaxArmor, HUMANS_DEFAULT_MAX_ARMOR)
...
public plugin_natives()
{
	...
	g_HumanClassShortDesc = ArrayCreate(32, 1)
	g_HumanClassHealth = ArrayCreate(1, 1)
	g_HumanClassMaxArmor = ArrayCreate(1, 1)
...
public show_menu_humanclass(id)
{
	...
	for (index = 0; index < g_HumanClassCount; index++)
	{
		...
		ArrayGetString(g_HumanClassShortDesc, index, shortDesc, charsmax(shortDesc))
...
public menu_humanclass(id, menuid, item)
{
	...
	g_HumanClassViewing[id] = index
	show_menu_info(id, g_HumanClassViewing[id])
...
public show_menu_info(id, class_index)
{
	...
	formatex(tittle, charsmax(tittle), "\y%L", id, "MENU_HCLASS")

	new name[32]
	new Float:maxspeed = Float:ArrayGetCell(g_HumanClassSpeed, class_index)
	ArrayGetString(g_HumanClassName, class_index, name, charsmax(name))
	
	// Info clase
	new health = ArrayGetCell(g_HumanClassHealth, class_index)
	new maxArmor = ArrayGetCell(g_HumanClassMaxArmor, class_index)
	new speed = cs_maxspeed_display_value(maxspeed)
	new Float:gravity = Float:ArrayGetCell(g_HumanClassGravity, class_index)

	ArrayGetString(g_HumanClassDesc, class_index, description, charsmax(description))
	...
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yAR: \r[\w%d\r]^n", maxArmor)
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yVelocidad: \r[\w%d\r]^n", speed)
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yGravedad: \r[\w%.2fx\r]^n^n", gravity)
	
	if (class_index == g_HumanClassNext[id])
	...
	if ((class_index + 1) >= g_HumanClassCount)
...
public menu_info(id, key)
{
	...
	switch (key)
	{
		case 0: 
		{
			if (g_HumanClassNext[id] != g_HumanClassViewing[id])
			{
				new index = g_HumanClassViewing[id]
				...
				// Obtener atributos
				new health = ArrayGetCell(g_HumanClassHealth, index)
				new maxArmor = ArrayGetCell(g_HumanClassMaxArmor, index)
				new Float:maxspeed = Float:ArrayGetCell(g_HumanClassSpeed, index)
				new Float:gravity = Float:ArrayGetCell(g_HumanClassGravity, index)
				new speed = cs_maxspeed_display_value(maxspeed)
				...
				zp_colored_print(id, "%L: %s | %s: %d | Armadura Máxima: %d | %s: %d | %s: %.2fx",
					id,
					"HUMAN_SELECT", name,
					szAttrib1, health,
					maxArmor,
		...
		case 1:
		{
			if ((g_HumanClassViewing[id] - 1) >= 0)
				g_HumanClassViewing[id]--
		}
		case 2:
		{
			if ((g_HumanClassViewing[id] + 1) < g_HumanClassCount)
				g_HumanClassViewing[id]++
		}
		case 4:
		{
...
public zp_fw_core_cure_post(id, attacker)
{
	...
	if (is_user_bot(id) || g_HumanClassNext[id] == ZP_INVALID_HUMAN_CLASS)
	{
		// Try choosing class
...
public _class_human_get_max_armor(plugin_id, num_params)
{
	...
	return ArrayGetCell(g_HumanClassMaxArmor, classid);
}
...
public native_class_human_register(plugin_id, num_params)
{
	...
	new maxArmor = get_param(4)
	new Float:speed = get_param_f(5)
	new Float:gravity = get_param_f(6)
	new shortDesc[32]
	get_string(7, shortDesc, charsmax(shortDesc))
	...
	// Short Description
	ArrayPushString(g_HumanClassShortDesc, shortDesc)
	...
	// Max Armor
	ArrayPushCell(g_HumanClassMaxArmor, maxArmor)
