...
new g_models_survivor_v_knife[MODEL_MAX_LENGTH] = "models/zombie_plague_chile/k/v_k_dragonclaws.mdl"
new g_models_survivor_p_knife[MODEL_MAX_LENGTH] = "models/zombie_plague_chile/k/p_k_dragonclaws.mdl"
...
public plugin_precache()
{
	...
	precache_model(g_models_survivor_p_knife)
	precache_model(g_models_survivor_v_knife)
}
...
public plugin_natives()
{
	...
	register_native("zp_class_survivor_strip_get", "native_class_survivor_strip_get")
	register_native("zp_class_survivor_strip_unset", "native_class_survivor_strip_unsetset")
}
...
public checkModel(id)
{
    ...
    if (get_user_weapon(id) == CSW_KNIFE && flag_get(g_IsSurvivor, id))
    {
        set_pev(id, pev_viewmodel2, g_models_survivor_v_knife);
        set_pev(id, pev_weaponmodel2, g_models_survivor_p_knife);
    }
    return PLUGIN_HANDLED;
}
...
public native_class_survivor_strip_get(plugin_id, num_params)
{
	new id = get_param(1)
	...
	return flag_get_boolean(g_Strip, id);
}

public native_class_survivor_strip_unsetset(plugin_id, num_params)
{
	new id = get_param(1)
	...
	if (flag_get(g_IsSurvivor, id))
	{
		log_error(AMX_ERR_NATIVE, "[ZP] Player already a Strip moment (%d)", id)
		return false;
	}
	flag_unset(g_Strip, id)
	return true;
