...
#define ZOMBIES_DEFAULT_SHORT_DESCRIPTION "Default"
#define ZOMBIES_DEFAULT_HEALTH 2250
#define ZOMBIES_DEFAULT_SPEED 0.95
#define ZOMBIES_DEFAULT_GRAVITY 1.0
#define ZOMBIES_DEFAULT_JUMP 270
#define ZOMBIES_DEFAULT_MODEL "zombie_source"
...
new g_ZombieClassViewing[MAX_PLAYERS + 1]
...
public plugin_init()
{
	...
	register_menu("Info Menu", KEYSMENU, "menu_info")
...
public plugin_cfg()
{
	...
	if (g_ZombieClassCount < 1)
	{
		...
		ArrayPushString(g_ZombieClassShortDesc, ZOMBIES_DEFAULT_SHORT_DESCRIPTION)
		...
		ArrayPushCell(g_ZombieClassJump, ZOMBIES_DEFAULT_JUMP)
...
public plugin_natives()
{
	...
	register_native("zp_class_zombie_get_jump", "_class_zombie_get_jump")
...
{
	static menu[128], name[32], shortDesc[64]
	...
	for (index = 0; index < g_ZombieClassCount; index++)
	{
		...
		ArrayGetString(g_ZombieClassShortDesc, index, shortDesc, charsmax(shortDesc))
		
		// Class available to player?
		if (g_ForwardResult >= ZP_CLASS_NOT_AVAILABLE)
			formatex(menu, charsmax(menu), "\d%s \r[\d%s\r]", name, shortDesc)
		// Class is current class?
		else if (index == g_ZombieClassNext[id])
			formatex(menu, charsmax(menu), "\r%s \r[\y%s\r]", name, shortDesc)
		else
			formatex(menu, charsmax(menu), "%s \r[\y%s\r]", name, shortDesc)
...
public menu_zombieclass(id, menuid, item)
{
	...
	g_ZombieClassViewing[id] = index
	show_menu_info(id, g_ZombieClassViewing[id])

	return PLUGIN_HANDLED;
}

public show_menu_info(id, class_index)
{
	static menu[512], tittle[128], description[256]
	new len

	formatex(tittle, charsmax(tittle), "\y%L", id, "MENU_ZCLASS")

	// Info clase
	new health = ArrayGetCell(g_ZombieClassHealth, class_index)
	new knockback = ArrayGetCell(g_ZombieClassKnockback, class_index)
	new Float:maxspeed = Float:ArrayGetCell(g_ZombieClassSpeed, class_index)
	new speed = cs_maxspeed_display_value(maxspeed)
	new Float:gravity = Float:ArrayGetCell(g_ZombieClassGravity, class_index)
	new salto = ArrayGetCell(g_ZombieClassJump, class_index)
	new name[32]
	ArrayGetString(g_ZombieClassName, class_index, name, charsmax(name))

	ArrayGetString(g_ZombieClassDesc, class_index, description, charsmax(description))

	len += formatex(menu[len], charsmax(menu) - len, "%s ^n^n", tittle)

	len += formatex(menu[len], charsmax(menu) - len, "%s ^n^n", description)

	len += formatex(menu[len], charsmax(menu) - len, "\r- \yClase Seleccionada: \r[\w%s\r]^n", name)
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yHP: \r[\w%d\r]^n", health)
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yVelocidad: \r[\w%d\r]^n", speed)
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yGravedad: \r[\w%.2fx\r]^n", gravity)
	len += formatex(menu[len], charsmax(menu) - len, "\r- \ySalto: \r[\w%d\r]^n", (salto))
	len += formatex(menu[len], charsmax(menu) - len, "\r- \yKB: \r[\w%.2fx\r]^n^n", knockback)
	
	if (class_index == g_ZombieClassNext[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r1. \dSeleccionar^n^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r1. \wSeleccionar^n^n")
	
	if ((class_index - 1) < 0)
		len += formatex(menu[len], charsmax(menu) - len, "\r2. \dAnterior^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r2. \wAnterior^n")


	if ((class_index + 1) >= g_ZombieClassCount)
		len += formatex(menu[len], charsmax(menu) - len, "\r3. \dSiguiente^n^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r3. \wSiguiente^n^n")

	len += formatex(menu[len], charsmax(menu) - len, "\r5. \wSalir")

	//server_print("[DEBUG] Longitud del menú: %d", len)

	// Fix for AMXX custom menus
	set_pdata_int(id, OFFSET_CSMENUCODE, 0)
	show_menu(id, KEYSMENU, menu, -1, "Info Menu")
}
...
public menu_info(id, key)
{
	...
	switch (key)
	{
		case 0: 
		{
			if (g_ZombieClassNext[id] != g_ZombieClassViewing[id])
			{
				new index = g_ZombieClassViewing[id]

				// Ejecutar forward pre-selección
				ExecuteForward(g_Forwards[FW_CLASS_SELECT_PRE], g_ForwardResult, id, index)
				
				if (g_ForwardResult >= ZP_CLASS_NOT_AVAILABLE)
					return PLUGIN_HANDLED;

				// Establecer clase como la siguiente para el jugador
				g_ZombieClassNext[id] = index
				
				// Nombre con soporte ML
				new name[32]
				ArrayGetString(g_ZombieClassName, index, name, charsmax(name))

				// Info clase
				new health = ArrayGetCell(g_ZombieClassHealth, index)
				new Float:knockback = ArrayGetCell(g_ZombieClassKnockback, index)
				new Float:gravity = Float:ArrayGetCell(g_ZombieClassGravity, index)
				new salto = ArrayGetCell(g_ZombieClassJump, index)
				new Float:maxspeed = Float:ArrayGetCell(g_ZombieClassSpeed, index)
				new speed = cs_maxspeed_display_value(maxspeed)

				// Traducciones de atributos
				new szAttrib1[32], szAttrib2[32], szAttrib3[32]
				format(szAttrib1, charsmax(szAttrib1), "%L", id, "ZOMBIE_ATTRIB1")
				format(szAttrib2, charsmax(szAttrib2), "%L", id, "ZOMBIE_ATTRIB2")
				format(szAttrib3, charsmax(szAttrib3), "%L", id, "ZOMBIE_ATTRIB3")

				// Mensaje de confirmación
				zp_colored_print(id, "%L: %s | %s: %d | %s: %d | %s: %.2fx | Salto: %d | KB: %.2fx",
					id,
					"ZOMBIE_SELECT", name,
					szAttrib1, health,
					szAttrib2, speed,
				...
				// Ejecutar forward post-selección
		...
		case 1:
		{
			if ((g_ZombieClassViewing[id] - 1) >= 0)
				g_ZombieClassViewing[id]--
		}
		case 2:
		{
			if ((g_ZombieClassViewing[id] + 1) < g_ZombieClassCount)
				g_ZombieClassViewing[id]++
		}
		case 4:
		{
			// Salir: no mostrar menú de nuevo
			return PLUGIN_HANDLED;
		}
	}

	// Volver a mostrar el menú con la clase actual
	show_menu_info(id, g_ZombieClassViewing[id])
	return PLUGIN_HANDLED;
}
...
public zp_fw_core_infect_post(id, attacker)
{
	...
	if (is_user_bot(id) || g_ZombieClassNext[id] == ZP_INVALID_ZOMBIE_CLASS)
	{
		// Try choosing class
...
public _class_zombie_get_jump(plugin_id, num_params)
{
	new id = get_param(1)
	...
	return ArrayGetCell(g_ZombieClassJump, classid);
}
...
public native_class_zombie_register(plugin_id, num_params)
{
	...
	new shortDesc[32]
	get_string(6, shortDesc, charsmax(shortDesc))
	new salto = get_param(7)
	...
	// Short Description
	ArrayPushString(g_ZombieClassShortDesc, shortDesc)
	...
	// Gravity
	ArrayPushCell(g_ZombieClassJump, salto)
