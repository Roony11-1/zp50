	...
	if (is_user_connected(attacker))
	{
		...
		{		
			...
			if (get_pcvar_num(cvar_cure_show_hud))
			{
				...
				static szMsg[128]
				format(szMsg, charsmax(szMsg), "%L", LANG_PLAYER, "NOTICE_ANTIDOTE", victim_name)
		...
		else
		{
			...
			if (get_pcvar_num(cvar_cure_show_hud))
			{
				...
				get_user_name(id, victim_name, charsmax(victim_name))
				static szMsg[128]
				format(szMsg, charsmax(szMsg), "%L", LANG_PLAYER, "NOTICE_ANTIDOTE2", attacker_name, victim_name)
				set_hudmessage(HUD_CURE_R, HUD_CURE_G, HUD_CURE_B, HUD_CURE_X, HUD_CURE_Y, 0, 0.0, 5.0, 1.0, 1.0, -1)
				ShowSyncHudMsg(0, g_syncHud, szMsg)
			}
		}
