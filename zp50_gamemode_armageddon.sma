...
new cvar_nemesis_aura, cvar_survivor_aura
...
public zp_fw_gamemodes_start()
{
	...
	new zombie_random = random_num(1, 2) // randon between all zombie super clases
	new human_random = random_num(1, 1) // random between all human super clases
	new human[33], zombie[33] // strings para el texto

	set_pcvar_num(cvar_nemesis_aura, 0)
	set_pcvar_num(cvar_survivor_aura, 0)

	// Decidir al azar entre quienes se juega
	...
	while (iSurvivors < iMaxSurvivors)
	{
		...
		switch(human_random)
		{
			case 1: zp_class_survivor_set(id)
		}
		iSurvivors++
	...
	while (iNemesis < iMaxNemesis)
	{
		...
		// If not, turn him into one
		switch(zombie_random)
		{
			case 1: zp_class_nemesis_set(id)

			case 2: zp_class_depredador_set(id)
	...
	{
		formatex(human, charsmax(human), "Survivors")

		switch (human_random)
		{
			case 1: formatex(human, charsmax(human), "Survivors")
		}

		switch (zombie_random)
		{
			case 1: formatex(zombie, charsmax(zombie), "Nemesis")
			case 2: formatex(zombie, charsmax(zombie), "Depredadores")
		}

		static szMsg[128]
		format(szMsg, charsmax(szMsg), "%s V/S %s", zombie, human)
