...
new cvar_plague_depredador_count, cvar_plague_depre_hp_multi
...
public plugin_precache()
{
	...
	cvar_plague_depredador_count = register_cvar("zp_plague_depredador_count", "1")
	cvar_plague_depre_hp_multi = register_cvar("zp_plague_depre_hp_multi", "1")
...
public zp_fw_gamemodes_choose_pre(game_mode_id, skipchecks)
{
	...
	if (alive_count < get_pcvar_num(cvar_plague_nemesis_count) + get_pcvar_num(cvar_plague_survivor_count) + get_pcvar_num(cvar_plague_depredador_count))
		return PLUGIN_HANDLED;
...
public zp_fw_gamemodes_start()
{
	...
	new depredador_count = get_pcvar_num(cvar_plague_depredador_count)
	new zombie_count = floatround((alive_count - (nemesis_count + survivor_count + depredador_count)) * get_pcvar_float(cvar_plague_ratio), floatround_ceil)
	...
	// Turn specified amount of players into Nemesis
	new iDepredador, iMaxDepredador = depredador_count
	while (iDepredador < iMaxDepredador)
	{
		...
		// Already a survivor or nemesis?
		if (zp_class_survivor_get(id) || zp_class_nemesis_get(id) || zp_class_depredador_get(id))
			continue;
		
		// If not, turn him into one
		zp_class_depredador_set(id)
		iDepredador++
		
		// Apply nemesis health multiplier
		set_user_health(id, floatround(get_user_health(id) * get_pcvar_float(cvar_plague_depre_hp_multi)))
	}
...
{
	static szMsg[128]
	format(szMsg, charsmax(szMsg), "%L", LANG_PLAYER, "NOTICE_PLAGUE")
