...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (g_AllowInfection && zp_core_is_zombie(attacker) && !zp_core_is_zombie(victim))
	{
		...
		// Nemesis shouldn't be infecting
		if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library) && zp_class_depredador_get(attacker))
			return HAM_IGNORED;
...
{
	static szMsg[128]
	format(szMsg, charsmax(szMsg), "%L", LANG_PLAYER, "NOTICE_VIRUS_FREE")
