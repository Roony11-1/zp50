...
native zp_has_balas(id)
native zp_has_recargas(id)
...
new cvar_survivor_unlimited_ammo
...
{
	if (!is_user_connected(msg_entity) || !is_user_alive(msg_entity) || zp_core_is_zombie(msg_entity))
		return;
	...
	// Retrasar el set_ammo para evitar pisar el vector de disparo
	static args[2]
	args[0] = msg_entity
	args[1] = weapon
	set_task(0.01, "refill_clip_ammo", msg_entity, args, sizeof args)

	// HUD muestra full clip
	set_msg_arg_int(3, get_msg_argtype(3), MAXCLIP[weapon])
}

// Nueva función que se ejecuta luego del delay
public refill_clip_ammo(const args[])
{
	new id = args[0]
	new weapon = args[1]
