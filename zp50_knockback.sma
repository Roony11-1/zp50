...
new cvar_knockback_nemesis, cvar_knockback_depredador
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library))
		cvar_knockback_depredador = register_cvar("zp_knockback_depredador", "0")
}
...
{
	if (equal(module, LIBRARY_NEMESIS) || equal(module, LIBRARY_DEPREDADOR))
...
public fw_TraceAttack_Post(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type)
{
	...
	// Depredador knockback disabled, nothing else to do here
	if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library) && zp_class_depredador_get(victim) && get_pcvar_float(cvar_knockback_depredador) == 0.0)
		return;
	...
	{	
		...
		if (zp_class_nemesis_mutated_get(victim))
			xs_vec_mul_scalar(direction, (get_pcvar_float(cvar_knockback_nemesis)-0.4), direction)
		else
