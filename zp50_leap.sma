...
new cvar_leap_depredador, cvar_leap_depredador_force, cvar_leap_depredador_height, cvar_leap_depredador_cooldown
...
public plugin_init()
{
	...
	{
		cvar_leap_depredador = register_cvar("zp_leap_depredador", "1")
		cvar_leap_depredador_force = register_cvar("zp_leap_depredador_force", "500")
		cvar_leap_depredador_height = register_cvar("zp_leap_depredador_height", "300")
		cvar_leap_depredador_cooldown = register_cvar("zp_leap_depredador_cooldown", "5.0")
	}
...
public fw_PlayerPreThink(id)
{
	...
	if (LibraryExists(LIBRARY_NEMESIS, LibType_Library) && zp_class_nemesis_get(id))
	{
		...
		if (zp_class_nemesis_mutated_get(id))
		{
			cooldown = get_pcvar_float(cvar_leap_nemesis_cooldown)
			force = get_pcvar_num(cvar_leap_nemesis_force)+300
			height = get_pcvar_float(cvar_leap_nemesis_height)+200
		}
		cooldown = get_pcvar_float(cvar_leap_nemesis_cooldown)
	...
	{
		// Check if nemesis should leap
		if (!get_pcvar_num(cvar_leap_depredador)) return;
		cooldown = get_pcvar_float(cvar_leap_depredador_cooldown)
		force = get_pcvar_num(cvar_leap_depredador_force)
		height = get_pcvar_float(cvar_leap_depredador_height)
	}
	// Survivor Class loaded?
