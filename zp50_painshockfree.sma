...
new cvar_painshockfree_zombie, cvar_painshockfree_human, cvar_painshockfree_nemesis, cvar_painshockfree_depredador, cvar_painshockfree_survivor
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library))
		cvar_painshockfree_depredador = register_cvar("zp_painshockfree_depredador", "1")
...
public fw_TakeDamage_Post(victim)
{
	...
	if (zp_core_is_zombie(victim))
	{
		...
		else if (LibraryExists(LIBRARY_DEPREDADOR, LibType_Library) && zp_class_depredador_get(victim))
		{
			if (!get_pcvar_num(cvar_painshockfree_depredador)) return;
		}
		else
		{
