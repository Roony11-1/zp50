...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
    ...
    // Nemesis Class loaded?
    if (zp_is_super_class(attacker) && get_pcvar_num(cvar_frags_nemesis_ignore))
    {
	...
	if (zp_core_is_zombie(attacker) && !zp_is_super_class(attacker))
	{
		// Curar atacante por matar humano
		HealPlayer(attacker, get_pcvar_num(cvar_infection_health_bonus));
	}
}

public zp_fw_core_infect_post(id, attacker)
{
	zombieMaxHealth[id] = get_user_health(id);
...
public HealPlayer(id, amount)
{
    ...
    new max_health = zombieMaxHealth[id]; // zombieMaxHealth debe estar bien definido y actualizado
