	...
	switch (game_mode_id)
	{
		case 0:
		{
			if (!zp_core_get_human_count())
				formatex(szMensaje, sizeof(szMensaje), "%L", LANG_PLAYER, "WIN_ZOMBIE");
			else if (!zp_core_get_zombie_count())
				formatex(szMensaje, sizeof(szMensaje), "%L", LANG_PLAYER, "WIN_HUMAN");
			else
				formatex(szMensaje, sizeof(szMensaje), "%L", LANG_PLAYER, "WIN_NO_ONE");
        }
        case 1:
        {
			if (!zp_core_get_human_count())
        ...
        {
			if (!zp_core_get_human_count())
				formatex(szMensaje, sizeof(szMensaje), "¡El Nemesis aniquiló a todos los humanos!");
			else if (!zp_core_get_zombie_count())
				formatex(szMensaje, sizeof(szMensaje), "¡Los humanos lograron vencer al Nemesis!");
			else
				formatex(szMensaje, sizeof(szMensaje), "¡La batalla entre el Nemesis y los humanos terminó sin un claro vencedor!");
        }
        case 4:
        {
			if (!zp_core_get_human_count())
				formatex(szMensaje, sizeof(szMensaje), "¡El Depredador cazó a todos los humanos sin piedad!");
			else if (!zp_core_get_zombie_count())
				formatex(szMensaje, sizeof(szMensaje), "¡Los humanos acabaron con el Depredador!");
			else
				formatex(szMensaje, sizeof(szMensaje), "¡El Depredador y los humanos no lograron derrotarse entre sí!");
        }
        case 5:
        {
			if (!zp_core_get_human_count())
				formatex(szMensaje, sizeof(szMensaje), "¡El Survivor fue derrotado por la horda!");
			else if (!zp_core_get_zombie_count())
				formatex(szMensaje, sizeof(szMensaje), "¡El Survivor erradicó la amenaza zombie!");
			else
				formatex(szMensaje, sizeof(szMensaje), "¡El Survivor sobrevivió, pero la batalla sigue inconclusa!");
        }
        case 6:
        {
			if (!zp_core_get_human_count())
        ...
        {
			if (!zp_core_get_human_count())
				formatex(szMensaje, sizeof(szMensaje), "¡Los Super Zombies aniquilaron a los Super Humanos!");
			else if (!zp_core_get_zombie_count())
				formatex(szMensaje, sizeof(szMensaje), "¡Los Super Humanos exterminaron a los Super Zombies!");
			else
				formatex(szMensaje, sizeof(szMensaje), "¡La batalla épica terminó en empate!");
        }
        default:
        {
			if (!zp_core_get_human_count())
